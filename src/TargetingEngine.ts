...
export function evaluateTargets(
  ...
  userId: string
): TargetEvaluationResult | null {
  if (!targets || targets.length === 0) {
    Logger.verbose("TargetingEngine", "No targets in manifest");
    return null;
  }
  ...
  Logger.verbose("TargetingEngine", `Evaluating ${sorted.length} targets for user ${userId.substring(0, 8)}...`);
 ...
 */
export function evaluateRules(rules: TargetRules, context: TargetingContext): boolean {
 ...
 */
export function evaluateRule(rule: TargetRule, context: TargetingContext): boolean {
  // Parse attribute path (e.g., "device.country" -> ["device", "country"])
  const parts = rule.attribute.split(".");
  if (parts.length !== 2) {
    Logger.verbose("TargetingEngine", `Invalid attribute format: ${rule.attribute}`);
    return false;
  }
  ...
  const categoryObj = context[category as keyof TargetingContext];
  if (!categoryObj || typeof categoryObj !== "object") {
    Logger.verbose("TargetingEngine", `Unknown category: ${category}`);
    return false;
  }

  const actualValue = (categoryObj as Record<string, unknown>)[attr];
...
function applyOperator(
  ...
  switch (operator) {
    ...
    default:
      Logger.verbose("TargetingEngine", `Unknown operator: ${operator}`);
      return false;
  }
}
 ...
 */
export function selectOnboardingByAllocation(
  onboardings: TargetOnboarding[],
  userId: string
): { onboarding: TargetOnboarding; bucket: number } {
